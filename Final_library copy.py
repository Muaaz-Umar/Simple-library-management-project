...
def borrowed_book_viewing(user_id):
    # Create a new window for book viewing
    book_viewing_window = tk.Toplevel(root)
...
def calculate_fine(borrow_date, return_date):
    ...
    max_fine_days = 7  # Maximum days for fine calculation
    ...
    if days <= max_fine_days:
        fine_amount = days * fine_rate
    else:
        fine_amount = max_fine_days * fine_rate
...
def fine_calculation(user_type, user_id):
    ...
    if borrowings:
        for i, borrowing in enumerate(borrowings):
            ...
            fine_amount = calculate_fine(borrow_date, return_date)
...
def student_main_screen(user_id):
    ...
    borrowed_books_button = tk.Button(
        root, text="View Borrowed Books", command=lambda: borrowed_book_viewing(user_id))
...
def student_signup():
    ...
    student_password_entry = tk.Entry(root)
